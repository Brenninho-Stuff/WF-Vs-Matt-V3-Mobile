...
class VisualsUISubState extends BaseOptionsMenu
{
	...
	public function new()
	{
		...
		{
			var note:StrumNote = new StrumNote(900 + (160*i*0.5), -200, i, 0);
		...
		var option:Option = new Option('Hold Splashes',
			"If unchecked, disables the cover and splash effect with holds",
			'holdSplashSetting',
			'string',
			['On', 'Only Covers', 'Off']);
		addOption(option);

		var option:Option = new Option('Note Highway Opacity',
			'How much transparent should the Note highway be.',
			'noteHighwayAlpha',
			'percent');
		option.scrollSpeed = 1.6;
		option.minValue = 0.0;
