	...
	function loadOptionSubstate()
	{
		switch (curIndex)
		{
			...
			case 1:
				...
				openSubState(substate);
			case 2:
				var path:String = Paths.getSharedPath('images/noteSkins/list.txt');
				var noteSkins:Array<String> = Assets.getText(path).trim().split('\n');

				if (!noteSkins.contains(ClientPrefs.data.noteSkin))
				{
					trace('WARN: Skin "' + ClientPrefs.data.noteSkin + '" not found. Reseting...');

					ClientPrefs.data.noteSkin = ClientPrefs.defaultData.noteSkin;
				}
				else
				{
					trace('Skin Found Sucessfully: ' + ClientPrefs.data.noteSkin);
				}
			...
			case 5:
				...
				@:privateAccess
				{
					...
					var warningText = new WiiCustomText(60, 400, FlxG.width, "Recommended to disable if sensitive to flashing lights! (or if your pc sucks)",
						1, "wii");
