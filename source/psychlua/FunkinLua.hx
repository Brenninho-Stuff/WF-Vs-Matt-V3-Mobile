...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		set('keyCount', PlayState.keyCount);
		...
		Lua_helper.add_callback(lua, "getMultikeyNoteIndex", function(index:Int) {
			return PlayState.multikeyIndexes[PlayState.keyCount][index];
		});
		...
		try{
			...
			if (Assets.exists(scriptName) /*&& !FileSystem.exists(scriptName)*/) {
