...
class ShaderFunctions
{
	public static function implement(funk:FunkinLua)
	{
		...
		funk.addLocalCallback("setCameraShaders", function(camera:String, shaderObjList:Array<String>) {
			if(!ClientPrefs.data.shaders) return false;

			var shaders:Array<BitmapFilter> = [];
			for (i in 0...shaderObjList.length)
			{
				var shader:FlxRuntimeShader = getShader(shaderObjList[i]);
				if (shader != null) {
					shaders.push(new ShaderFilter(shader));
				}
			}

			var cam = LuaUtils.cameraFromString(camera);
			cam.filters = shaders;
